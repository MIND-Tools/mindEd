...
public class FractalJavaValidator extends AbstractFractalJavaValidator {
	...
	/**
	 * Assert that an implementation file associated
	 * to a primitive exists. 
	 * 
	 * @param implementationDefinition
	 */
	@Check
	public void existingImplementationFile(ImplementationDefinition implementationDefinition){
		
		FileC fileC = implementationDefinition.getFileC();
		
		// the file extension should be ".c"
		if (! fileC.getFileName().endsWith(".c"))
			warning("The file extension should be \".c\" .", AdlPackage.IMPLEMENTATION_DEFINITION__FILE_C);
		...
		for (MindFile file : files) {
			// here we are
			if (file.getName().equals(fileC.getFileName())){
				// the file exist, good bye
				return;
			}
		}
		// we don't find it -> warning at least
		warning("This source file cannot be found.", AdlPackage.IMPLEMENTATION_DEFINITION__FILE_C);
	 ...
	 */
	@Check
	public void assertValideFQN(CompositeComponentDefinitionCustomImpl componentDefinitionCustomImpl){

		
		String expectedFQN = FractalUtil.getFQNFromAdlDefinition((AdlDefinitionImpl) componentDefinitionCustomImpl.eContainer());
		String FQN = componentDefinitionCustomImpl.getName();
		
		if (! FQN.equals(expectedFQN)){
			error("Composite should be named: " + expectedFQN , AdlPackage.COMPOSITE_COMPONENT_DEFINITION__NAME_FQN);
		}
	...
	public void assertValidExtendGeneric(ComponentReference componentReference){
		...
		ArchitectureDefinition architectureDefinition = FractalUtil.getArchitecureDefinitionFromChild(componentReference);
			
		
		// -- Retrieve fully qualified name from reference name		
		String referenceName = componentReference.getReferenceName();
		String extendFQN = FractalUtil.getFQNFromPartialName(architectureDefinition, referenceName);
		// --
		...
		if (extendFQN != null) {			
			boolean isValid = adlMergeUtil.isValidDefinitionForMerge(architectureDefinition, extendFQN );
			if (isValid){				
				return;
			}
		}
	...
	/**
	 * 
	 * Assert that there i no problem for this composite definition to extend another one
	 * 
	 * @param compositeReferenceDefinitionCustomImpl
	 */
	@Check
	public void assertValidExtendComposite(CompositeReferenceDefinition compositeReferenceComponentDefinition){		
		this.assertValidExtendGeneric( (ComponentReference) compositeReferenceComponentDefinition);
		ArchitectureDefinition a = FractalUtil.getArchitecureDefinitionFromChild(compositeReferenceComponentDefinition);
	...
	/**
	 * 
	 * Assert that there i no problem for this primitive definition to extend another one
	 * 
	 * @param compositeReferenceDefinitionCustomImpl
	 */
	@Check
	public void assertValidExtendComposite(PrimitiveReferenceDefinition primitiveReferenceComponentDefinition){		
		this.assertValidExtendGeneric( (ComponentReference) primitiveReferenceComponentDefinition);
	}
	
	/**
	 * 
	 * Assert that there i no problem for this type definition to extend another one
	 * 
	 * @param compositeReferenceDefinitionCustomImpl
	 */
	@Check
	public void assertValidExtendComposite(TypeReferenceDefinition typeReferenceComponentDefinition){		
